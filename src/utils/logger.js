...
const ACTION_EMOJIS = {
  ...
  automod_word: '🤬', automod_link: '🔗', automod_discord_link: '🚫',
  message_edit: '✏️', message_delete: '🗑️', member_join: '👋', role_update: '🎭',
};
...
async function logAction(guild, action, { moderator, target, reason, extra }) {
  ...
  if (extra) for (const [k, v] of Object.entries(extra)) embed.addFields({ name: k, value: String(v), inline: true });
