...
const COLORS = {
  background: '#1a1a2e', backgroundAlt: '#16213e',
  accent: '#7c3aed', accentLight: '#a78bfa',
  barBg: '#2d2d4e', barFill: '#7c3aed', barFillEnd: '#06b6d4',
...
async function generateLevelCard({ user, xp, nextLevelXp, level, rank, totalXp }) {
  ...
  try {
    ...
    ctx.strokeStyle = COLORS.avatarBorder; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.arc(avatarX + avatarRadius, avatarY, avatarRadius, 0, Math.PI * 2); ctx.stroke();
    ctx.shadowBlur = 0;
    ctx.save(); ctx.beginPath();
    ctx.arc(avatarX + avatarRadius, avatarY, avatarRadius - 4, 0, Math.PI * 2); ctx.clip();
    ...
    ctx.arc(avatarX + avatarRadius, avatarY, avatarRadius - 4, 0, Math.PI * 2); ctx.fill();
  }
  ...
  if (progress > 0) {
    ...
    fillGrad.addColorStop(0, COLORS.barFill); fillGrad.addColorStop(1, COLORS.barFillEnd);
    ctx.fillStyle = fillGrad; roundRect(ctx, barX, barY, Math.max(barWidth * progress, 28), barHeight, 14); ctx.fill();
    ctx.shadowColor = COLORS.accent; ctx.shadowBlur = 10;
    ctx.fillStyle = 'rgba(255,255,255,0.1)';
    roundRect(ctx, barX, barY, Math.max(barWidth * progress, 28), barHeight / 2, [14, 14, 0, 0]); ctx.fill();
    ctx.shadowBlur = 0;
  }
  ...
  milestones.forEach((milestone, i) => {
    ...
    if (reached) { badgeGrad.addColorStop(0, COLORS.accentLight); badgeGrad.addColorStop(1, COLORS.accent); ctx.fillStyle = badgeGrad; ctx.shadowColor = COLORS.accent; ctx.shadowBlur = 8; }
    else { ctx.fillStyle = COLORS.barBg; ctx.shadowBlur = 0; }
    ctx.beginPath(); ctx.arc(bx + 14, badgeY - 14, 14, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
    ctx.fillStyle = reached ? '#ffffff' : COLORS.textMuted; ctx.textAlign = 'center';
    ctx.fillText(milestone, bx + 14, badgeY - 9); ctx.textAlign = 'left';
  });
  ctx.font = '13px sans-serif'; ctx.fillStyle = COLORS.textMuted;
...
function drawStat(ctx, x, y, label, value, valueColor) {
  ctx.font = '13px sans-serif'; ctx.fillStyle = '#6b6b80'; ctx.fillText(label, x, y);
  ctx.font = 'bold 28px sans-serif'; ctx.fillStyle = valueColor; ctx.fillText(value, x, y + 30);
}
